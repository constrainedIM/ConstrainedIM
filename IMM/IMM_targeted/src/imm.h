...
*/
            ...
            //R/=100;
	    //INFO("before build protected hyper graph");
            g.build_protected_hyper_graph_r(R, arg);
            //INFO("built protected hyper graph");
			double opt = g.build_seedset(arg.k)*g.protected_group_size;
            return opt;
        }
		...
		double ept = g.test_seedset(arg.k);
                ...
                if (ept > 1 / pow(2.0, x))
                {
                    ...
                    //INFO("step3", OPT_prime);
                    //INFO("step3", OPT_prime * (1+epsilon_prime));
                    return OPT_prime;
                }
            }
            ASSERT(false);
    ...
    public:
	    ...
	    double opt_lower_bound;
            opt_lower_bound = step3(g, arg ); //estimate OPT
