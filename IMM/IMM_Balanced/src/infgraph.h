...
public:
    ...
    void readSeed_set()
    {
        ifstream cin((folder + "seed_set.txt").c_str());
        ASSERT(!cin == false);
        string s;
        while (cin >> s)
        {
		...
		s.erase(s.length() - 1);
            ...
            seedSet.push_back(stoi(s));
            //ASSERT(false);
        }
        TRACE(protected_group_size);
        cin.close();
    }
    ...
    void build_protected_hyper_graph_r(int64 R, const Argument & arg)
    {
        if( R > INT_MAX ){
            cout<<"Error:R too large"<<endl;
            exit(1);
	...
	init_hyper_graph();
        ...
        for (int i = 0; i < R; i++)
        {
            random_number.push_back(  sfmt_genrand_uint32(&sfmtSeed) % protected_group_size);
        }
