...
class Imm
{
    private:
	  ...
	  double opt = g.test_seedset(arg.k)*g.n;
            return opt;
        }
 ...
 static double step4(InfGraph &g, const Argument & arg, double OPT_prime)
        {
            Timer t(4, "step4");
            ASSERT(OPT_prime > 0);
            double e = exp(1);
            double alpha = sqrt(log(g.protected_group_size) + log(2));
            double beta = sqrt((1-1/e) * (Math::logcnk(g.protected_group_size, arg.k) + log(g.protected_group_size) + log(2)));

            int64 R = 2.0 * g.protected_group_size *  sqr((1-1/e) * alpha + beta) /  OPT_prime / arg.epsilon / arg.epsilon ;

            //R/=100;
            g.build_protected_hyper_graph_r(R, arg);
            
			double opt = g.test_seedset(arg.k)*g.protected_group_size;
            return opt;
        }
    ...
    public:
	    ...
	    INFO("########## Step3 ##########");
            ...
            OPT_prime = step3(g, arg ); //estimate OPT
	    //INFO(protected_opt_lower_bound);

            INFO("########## Step4 ##########");

            double protected_opt_lower_bound = OPT_prime;
            INFO(protected_opt_lower_bound);
            step4(g, arg, OPT_prime);
            //INFO("step2 finish");
