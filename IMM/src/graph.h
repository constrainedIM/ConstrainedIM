...
{
public:
    int n, m, k, protected_group_size;
    vector<int> inDeg;
    ...
    void readProtected_group_size()
    {
        ifstream cin((folder + "protected_group.txt").c_str());
        if(!cin == false){
        	string s;
        	while (cin >> s)
        	{
            		if (s.substr(0, 2) == "R=")
            		{
                		protected_group_size = atoi(s.substr(2).c_str());
                		break;
            		}
            		ASSERT(false);
        	}
